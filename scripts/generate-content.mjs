...
function collectBlogPosts(markdown, metadata) {
  ...
  for (const [key, source] of Object.entries(markdown)) {
    ...
    const seoTitle = metadata[key]?.seo_title || title;
    const seoDescription = metadata[key]?.seo_description || summary;
...
export async function generateContent() {
  ...
  const source = [
    ...
    `export const markdownMetadata: Record<string, { published?: string; updated?: string; summary?: string; seo_title?: string; seo_description?: string; topic?: string; draft?: string }> = ${JSON.stringify(metadata, null, 2)};`,
    "export type GeneratedBlogPost = { language: \"zh\" | \"en\"; slug: string; href: string; contentPath: string; title: string; summary: string; seoTitle: string; seoDescription: string; date: string; published?: string; updated?: string; topic: string; readingMinutes: number; available: true };",
