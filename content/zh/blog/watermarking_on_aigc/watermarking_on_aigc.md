...
近日，为响应欧盟的监管要求，[Anthropic宣布将在旗下AI模型的输出中引入隐形文本水印](https://www.anthropic.com/news/claude-text-watermark)。此举旨在不影响用户阅读体验的前提下，为AI生成内容（AIGC）提供可靠的溯源与检测手段。
...
具体到 KGW 算法中：如果一段文本**没有**添加水印，那么在常用的零假设模型下，我们将各位置是否命中“绿名单”近似视为相互独立、命中概率固定的伯努利试验（假设绿名单的划分比例为 25%）。
设整段文本的总词元数为 $n$，其中属于绿名单的词元数量为$x$。在上述独立性近似下，$x$服从二项分布（如图3所示）。当$n$足够大时，可以通过中心极限定理对其进行标准化，计算出$Z_{score} = \frac{x - np}{\sqrt{np(1-p)}}$，此时它近似服从标准正态分布。
...
因此，业界正在探索**不改变文本原始概率分布的非失真水印技术**（non-distortionary watermarking，也常被直观地称为无损或无偏水印）。这正是我们[下一篇博客的主题：**Google DeepMind SynthID-Text**](https://gentang.github.io/zh/blog/watermarking_on_aigc_2/)。
