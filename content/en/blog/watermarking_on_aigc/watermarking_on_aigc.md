...
Anthropic recently announced that it will introduce invisible text watermarks into the outputs of its AI models in response to European regulatory requirements. The goal is to provide a reliable way to trace and detect AI-generated content (AIGC) without disrupting the reading experience.
...
KGW's central limitation is that it **changes the model's original token distribution**. The probability of a watermarked output differs from the probability assigned by the unmodified model. This distortion may be acceptable for open-ended text such as conversation or prose, but it can be costly in settings with strict structural or logical constraints, including code, mathematical notation, and JSON.

Researchers are therefore exploring watermarking methods that preserve the model's original output distribution—often described as distortion-free or unbiased watermarks. That is the subject of the next article: **Google DeepMind SynthID-Text**.
