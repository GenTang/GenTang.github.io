...
- `grad_wrt`: Records the node's partial derivative with respect to a predecessor node. In Figure 7-2, for example, if the current node is $o_i$, this attribute stores the value of $\partial o_i/\partial a$.
...
![Figure 7-6 | 88%](./images/7-6.webp)

To clarify the details further, Figure 7-7 visualizes the state of every node as the algorithm runs. The process can be followed step by step. First, set the root's initial gradient to 1 and begin propagating level by level. An operation node accumulates gradients from downstream nodes and passes them backward according to the local derivative rule for its operation. A variable node only needs to accumulate its gradient.

![Figure 7-7 | 88%](./images/7-7.webp)
...
![Figure 7-8 | 80%](./images/7-8.webp)

This efficient method of gradient calculation is important in many fields and is particularly powerful for neural networks. It provides an essential foundation for model training, especially when a neural network has an enormous number of parameters and a complex layered structure. Backpropagation dramatically accelerates neural network training.
