...
Although PyTorch implements backpropagation extremely well, a production-grade implementation must address many engineering concerns beyond the algorithm's core steps, including fault tolerance, generality, and complex matrix operations.

This comprehensive implementation is a double-edged sword. On the one hand, it is easy to use and calculates gradients efficiently. On the other, the many details can obscure the algorithm's central idea for beginners reading the code, leaving them lost in a dense forest of complexity.
...
3. **Directed edges**: If a basic operation produces variable $y$ from variable $x$, the graph contains a directed edge from $x$ to $y$. Conversely, such an edge indicates that $y$ was derived from $x$ through an operation. The definition of directed edges is therefore closely related to the basic operations supported by the graph.
