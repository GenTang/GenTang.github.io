...
![Figure 11-31 | 65%](./images/11-31.png)
...
### 11.5.4 If We Rebuilt ChatGPT

If we set out to rebuild a ChatGPT proficient in Chinese, we would technically stand at the frontier of the discipline and possess nearly all the tools required[^11-5-11]. What practical challenges might we encounter? This section offers several of the author's thoughts as inspiration.

From the perspective of implementation, training a production-ready large language model requires exceptional engineering skill to keep the process running smoothly. Although extremely challenging, this is not an insurmountable technical barrier. At the time of writing in 2023, a serious chip shortage caused by various factors affects training speed and may even prevent model training from being completed. Fortunately, distributed-computing frameworks can mitigate the computing crisis through large clusters. Throughout the history of computing, the most advanced chips have rapidly become widely accessible, and market forces have driven technology around many restrictions. The author therefore believes that the computing crisis will not last long.

From the perspective of data, open-source Chinese corpora are highly limited and their quality needs improvement. Although Chinese is widely used in the physical world, relatively little material has been preserved digitally. This creates a negative feedback loop: scarce Chinese material leads both to insufficient research on Chinese and to fewer attempts to train models in the language.

From the perspective of language, we want the model to master Chinese, but learning only Chinese would limit its performance. Most scientific knowledge exists in English, and Chinese does not encode enough of it—not because English is superior, but because historical circumstances made English the language of science. This appears insoluble, but large language models can translate or automatically generate linguistic material. Using technology to enrich Chinese corpora therefore seems worth exploring.

LLMs are expensive to train and consume substantial resources during inference. Constrained by their architectures and training methods, they are not artificial general intelligence and cannot perform every task. Sustainable development requires finding applications in which they can be effective. Practical applications not only fund projects but also generate new applications and propel technical progress. ChatGPT is a successful example. The original Transformer was designed by a Google team, and several models achieved similar results at the time. ChatGPT found a suitable question-answering application and deeply fine-tuned the Transformer. It rapidly became a popular tool, which in turn further accelerated its technical development.

In summary, linguistic resources and application scenarios are important constraints and difficult problems when rebuilding ChatGPT, and no mature solutions yet exist.
...
[^11-5-11]: Strictly speaking, two technical tools remain to be perfected: the process for fine-tuning models with reinforcement learning and more advanced architectures such as GPT-4.
