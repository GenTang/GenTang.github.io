...
Most neural-network parameters come from linear layers[^11-4-6], so we need consider only how to fine-tune a linear layer efficiently. Figure 11-23 constructs three different linear models.
...
- Use `target_modules`, shown in lines 10 and 11, to specify the linear layers to which LoRA adapters should be added. Multiple adapters can be added to a model as needed.
