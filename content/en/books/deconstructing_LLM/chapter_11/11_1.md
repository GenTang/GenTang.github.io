...
![Figure 11-2 | 80%](./images/11-2.webp)
...
Taken together, Equations (11-1) through (11-3) reveal that the essence of attention is feature extraction: finding a better feature representation of the text, namely the context vector. Although the hidden state at each step represents the text well, it also participates in computing the weights. This dual role can prevent it from performing either task optimally and impair model performance. To solve the problem, we temporarily leave the recurrent-neural-network framework and use the three equations above to design an improved attention mechanism.

The improved mechanism still distinguishes between a context sequence and a sequence being processed, but the two may now be the same. Introduce three vectors for each token: a query, key, and value, denoted Q, K, and V, respectively, as shown in Figure 11-3. The final feature representation is obtained as follows.

1. Use Q and K to compute alignment scores between tokens. More specifically, match the current token in the sequence being processed, represented by Q, with every token in the context sequence, represented by K, and take their inner products to obtain the alignment scores.
...
![Figure 11-3 | 72%](./images/11-3.webp)

The central steps of attention involve the three vectors Q, K, and V. Q encodes what the current token is looking for, K identifies the information associated with each token in the context, and V contains the features to be aggregated. In plain language, attention can therefore be understood as follows: the feature representation of a text is a weighted combination of token features, with the weights primarily reflecting relationships among tokens. This mechanism avoids a shortcoming of standard recurrent neural networks—their difficulty capturing relationships among tokens effectively; see the left side of Figure 11-3 and [Section 10.5.1](/books/deconstructing_LLM/chapter-10/10-5#section-10-5-1) for details. Notably, the formula for the context vector—see Equation (11-3)—strongly resembles the hidden state, so attention can also be regarded as a member of the recurrent-neural-network family.

The improved mechanism applies across a broader range of settings. When the processed sequence and context sequence differ, the algorithm is called cross-attention and is used in sequence-to-sequence mode. When they are the same, it is called self-attention and is used in autoregressive and autoencoding modes. The two modes differ as follows. In autoregressive mode, the model attends only to the current and preceding tokens; attention weights for later tokens—the dashed region in Figure 11-3—are forced to 0. This is called unidirectional attention. In autoencoding mode, attention has no such restriction; the dashed region is computed normally. This is called bidirectional attention.
...
- Scaling: An alignment score is the inner product of two vectors, so its variance increases with the key dimension. When the input to Softmax has high variance, the resulting weight distribution becomes overly concentrated at one position, as shown on the left of Figure 11-5, which is clearly undesirable. After the inner product is computed, the alignment scores must therefore be scaled to keep their variance under control.
...
![Figure 11-5 | 80%](./images/11-5.webp)
