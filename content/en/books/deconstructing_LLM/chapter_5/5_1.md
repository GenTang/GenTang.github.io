...
At its core, a model performs mathematical operations on data. For its computations to be meaningful, two conditions must hold. First, the data must be represented in a form that the model can process numerically. Second, the mathematical operations performed on the features (independent variables) must be meaningful and valid. Data used for modeling can generally be processed directly. In special settings such as natural language processing or image recognition, the raw data may require specific transformations, but these transformations are usually fairly straightforward.
...
- **Nominal categorical features** have no intrinsic order. Consider a variable representing regions, where 0 denotes Beijing, 1 Shanghai, 2 Shenzhen, and so on. Neither the numerical ordering nor arithmetic operations have any practical meaning.
