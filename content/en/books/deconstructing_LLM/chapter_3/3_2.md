...
This section implements linear regression with two open-source Python libraries: scikit-learn and statsmodels. Both implementations are straightforward. The scikit-learn version follows the machine-learning approach, whereas the statsmodels version emphasizes statistical analysis. In addition to fitting the model, we will calculate confidence intervals and conduct hypothesis tests.

### 3.2.1 Implementing the Machine-Learning Approach

Fitting a linear regression model with scikit-learn requires three main steps, as shown in Listing 3-1.

1. **Prepare the data**: Lines 6–8 split the data into training and test sets. The purpose is to prevent the model from overfitting, as discussed in detail in [Section 3.3.1](/books/deconstructing_LLM/chapter-3/3-3#section-3-3-1).
2. **Build and train the model**: Line 11 creates the model, and line 14 trains it. Calling `model.fit` estimates the model parameters from the supplied data and modifies the `model` object in place. Thus, although line 14 contains no explicit assignment, `model` now represents the trained model.
3. **Evaluate model performance**: Scikit-learn makes it easy to calculate the model's mean squared error and coefficient of determination, as shown in lines 17–20.
...
Plotting the model results produces Figure 3-8. The plot uses another widely used third-party library, matplotlib; see the accompanying code for implementation details.

![Figure 3-8 | 55%](./images/3-8.webp)
...
- The model's mean squared error is 0.726. This means that if the model is used to estimate total production cost, its root-mean-square error is $\sqrt{0.726}\approx0.85$.
- The model's coefficient of determination is 0.828. This means that the model explains approximately 83% of the variation in production cost in the sample. The remaining 17% is not explained by the current model and may reflect random variation or other factors not included in the model.

### 3.2.2 Implementing the Statistical-Analysis Approach

The statistical approach uses models to analyze data and then revises and improves them in light of the results. The implementation in this section demonstrates that process. We begin by using statsmodels to fit the linear regression model introduced earlier, $y_i=ax_i+b+\varepsilon_i$. See Listing 3-2 for the implementation.

1. Statsmodels represents linear regression in matrix form. To include the parameter $b$, we therefore add a column of ones to the design matrix. As line 8 shows, the `add_constant` function performs this operation and names the new column `const`.
2. Building and fitting the model is also straightforward, as shown in lines 11 and 12. Line 11 creates the linear regression model, and line 12 fits it and estimates its parameters.
3. For a fitted model, the `summary` method reports the most commonly used statistics, while `f_test` conducts hypothesis tests. See lines 14–24 for the code; the results are discussed in detail below.
...
We now examine the fitted model in detail. Running line 15 of Listing 3-2 produces the output shown in Figure 3-9, which reports the parameter estimates and their associated statistics.

![Figure 3-9 | 80%](./images/3-9.webp)
...
![Figure 3-10 | 70%](./images/3-10.webp)
