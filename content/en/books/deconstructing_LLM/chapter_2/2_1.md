...
- The matrix above describes the initial state of the four heroes. Their attribute data change during the game, so each level-up requires a new matrix to represent them. Arranging these matrices sequentially produces what is known as a tensor—more precisely, a three-dimensional tensor.

![Figure 2-1 | 78%](./images/2-1.webp)
...
#### 2. Three Forms of Multiplication Involving Matrices

(1) **Scalar multiplication**. Any real number can be multiplied by any matrix. Suppose $k$ is a real number. Its product with the matrix $\mathbf X$ is defined as follows:
...
Element-wise matrix multiplication is uncommon in mathematical theory, but it is frequently used in programming to compute many element-wise products in parallel.

![Figure 2-2 | 88%](./images/2-2.webp)

#### 3. Matrix Division: The Matrix Inverse
...
It follows directly from Equation (2-15) that the dot product of a vector with itself equals the square of its norm, as shown in Equation (2-16).
...
In addition to dot products and angles, vectors can have another important relationship: one vector can be expressed as a linear combination of others. As a simple example, the row vector $\mathbf X=(x_1,x_2,x_3)$ can be expressed as the linear combination

$$
\mathbf X=x_1(1,0,0)+x_2(0,1,0)+x_3(0,0,1)
$$

In other words, $\mathbf X$ lies in the span of the three row vectors $\mathbf e_1=(1,0,0)$, $\mathbf e_2=(0,1,0)$, and $\mathbf e_3=(0,0,1)$.

Linear combinations and spans are fundamental concepts with many applications in optimizing vector and matrix computations. This section examines their use in matrix decomposition. We begin by defining the **rank of a matrix**. Consider an $n\times m$ matrix, which can be viewed as a collection of $n$ row vectors. It can be proved mathematically that a smallest set of $r$ linearly independent row vectors[^linear-independence] spans every row of the matrix. This value $r$ is the rank of the matrix. Although a rigorous proof of this result is quite involved, there is no need to worry: the simple example on the left side of Figure 2-4 makes the idea intuitive. In this example, the matrix has rank 1 because every row vector is a scalar multiple of the vector inside the dashed box.
...
[^three-dimensions]: The physical world familiar to us is three-dimensional in space, so vectors of larger shape cannot be visualized directly. The intuition behind the angle definition can therefore be explained only by analogy with three-dimensional space.
