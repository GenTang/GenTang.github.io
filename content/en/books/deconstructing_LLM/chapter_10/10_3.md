...
![Figure 10-11 | 95%](./images/10-11.webp)

Vanilla neural networks can handle only the first category. Recurrent neural networks, the subject of this section, can handle all four. More importantly, they can adapt their data-transformation steps to the sequence and extract information more efficiently, delivering strong predictive performance on many tasks. We now examine this class of models in depth and explore its applications to natural language processing.
...
Recurrent neural networks break this limitation by introducing recurrent connections across time steps. In a diagram, the difference is the addition of a recurrent arrow. Recurrent neural networks are generally used for sequential data, and this arrow indicates that the neural block processes not only the current input but also information carried forward from preceding elements. Importantly, in a typical recurrent-neural-network diagram, the tensors represented by the output arrow and the recurrent arrow are actually the same—$h$ in the figure—and are generally called the hidden state. Hidden states form the core of the model and are the principal subject of the next section.

![Figure 10-12 | 75%](./images/10-12.webp)
...
![Figure 10-13 | 88%](./images/10-13.webp)
...
Understanding hidden states is the key to mastering recurrent neural networks. Neural-network development is an engineering discipline, and implementing a model in code is one of the best ways to understand it. Our task now is to translate the unfolded form of a recurrent neural network into code. Unlike the vanilla neural networks discussed earlier, the recurrent-neural-network diagram in Figure 10-13 has two features that may confuse beginners.
...
![Figure 10-14 | 95%](./images/10-14.webp)
...
The model used for this prediction is generally called the language-modeling head. It is usually a simple linear model that transforms a hidden-state vector of shape $(1,H)$ into a tensor of shape $(1,VS)$. The latter contains unnormalized token scores, or logits, which Softmax converts into a probability distribution over the vocabulary. The implementation is shown in Listing 10-4.
...
![Figure 10-15 | 85%](./images/10-15.webp)
...
![Figure 10-16 | 70%](./images/10-16.webp)

Training a recurrent neural network differs substantially from training a vanilla neural network. Training examples in a vanilla neural network are independent and can be processed in parallel with tensor operations, so the code is concise and contains almost no loops. Elements within a recurrent sequence depend on one another and cannot be computed in parallel. During training, predictions and model losses must therefore be produced step by step before backpropagation and gradient descent are performed. Listing 10-5 shows the implementation. The most important step is accumulating the model loss inside the loop, as shown in lines 11–13.
...
![Figure 10-17 | 90%](./images/10-17.webp)
...
[^10-3-4]: PyTorch provides recurrent neural networks through `nn.RNN`. Its linear-transformation component contains two bias terms. Conceptually, only one is needed; two are included for compatibility with cuDNN, the CUDA Deep Neural Network library used for GPU-accelerated neural-network computation. Similar handling occurs in other recurrent neural network models, such as long short-term memory networks.
