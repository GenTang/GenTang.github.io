...
A traditional multilayer perceptron, as a representative vanilla neural network, meets neither requirement. Its input shape is fixed, and its architecture cannot directly capture dependencies among characters. Carefully modifying the model input, however, can resolve—or partially resolve—both problems, making it possible to use a multilayer perceptron to learn language.
...
![Figure 10-7 | 75%](./images/10-7.webp)
...
![Figure 10-8 | 90%](./images/10-8.webp)
...
![Figure 10-9 | 90%](./images/10-9.webp)
...
On the basis of the preceding discussion, we can implement the simplified text embedding in Listing 10-2. The embedding operation requires only the token indices; unnecessary one-hot encoding can be omitted, as shown in lines 13–15. PyTorch also provides a built-in embedding module, `nn.Embedding`, which expects the same input format as Listing 10-2. This explains why the implementation in [Section 10.2.1](/books/deconstructing_LLM/chapter-10/10-2#section-10-2-1) does not include a one-hot-encoding step.
...
1. As line 11 shows, the input has shape $(B,10)$, where $B$ is the batch size and 10 is the window length. Each entry contains the corresponding token's index in the vocabulary; the result in Figure 10-8 may help clarify this point.
2. The embedding dimension is 30, meaning that 30 features describe each token. After embedding, the data therefore has shape $(B,10,30)$, as shown in line 12.
...
Finally, from the perspective of learning efficiency, although the text windows in the training data overlap, the model treats them as independent training examples. In other words, the information in overlapping regions is recomputed each time, wasting substantial computational resources and reducing training efficiency.
