...
We can formalize this description as a model. Suppose the independent variables $\mathbf X_i=(x_{i,1},x_{i,2},\cdots,x_{i,k},1)$ determine customer $i$'s utility from buying the garment[^intercept], including positive utility $y_i^*$, which makes the customer happy, and negative utility $\tilde y_i$, which causes inconvenience. Let $y_i$ represent the purchase decision: $y_i=1$ means that customer $i$ buys the garment, while $y_i=0$ means that the customer does not. Then
