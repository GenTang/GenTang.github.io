...
Practical applications often involve multiclass classification, in which there are more than two classes to predict. On an e-commerce website, for example, customers might be divided into regular customers, premium customers, and customers at risk of churn. Can logistic regression handle such multiclass problems? Yes, but the model must be adjusted appropriately. There are two common approaches. One changes the model structure to produce **multinomial logistic regression**. The other leaves the model structure unchanged but adds steps that transform the multiclass problem into multiple binary-classification problems; the classic example is **one-vs-rest** (OvR), also known as one-vs-all[^one-vs-one]. The following sections discuss both approaches in detail.
...
Here, $Y_{i,j}^*$ is the latent variable for observation $i$ and class $j$, which can be understood as the utility of class $j$ for observation $i$; $\mathbf X_i$ contains the features of observation $i$; $\boldsymbol\theta_j$ contains the parameters of the corresponding linear model; and $\varepsilon_j$ is a random disturbance following the standard Type I extreme value distribution[^extreme-value], written as $\varepsilon_j\sim EV_1(0,1)$.

As in the binary derivation, observation $m$ belongs to class $l$ if and only if the utility of class $l$ for that observation exceeds the utilities of all other classes—that is, $Y_{m,l}^*=\max_jY_{m,j}^*$. Let $Y_m=l$ mean that observation $m$ belongs to class $l$. Then
...
### 4.5.2 One-vs-Rest: From Binary to Multiclass

Although a multiclass problem involves several classes, if we focus on one particular class—class 1, for example—each observation either belongs to class 1 or does not[^ova-imbalance]. This is the familiar binary-classification problem.

Extending this reasoning to every class produces the one-vs-rest strategy. Specifically, suppose the multiclass problem has $k$ classes, denoted by $0,1,\cdots,k-1$. Build an independent model for each class, decomposing the original $k$-class problem into $k$ binary-classification problems. Continuing the notation above, let $Y_i=j$ mean that observation $i$ belongs to class $j$, and generate $k$ new label variables:
...
Fit a separate logistic regression to each dataset $\{\mathbf X_i,\tilde Y_{i,j}\}$. The corresponding prediction—the probability that observation $i$ belongs to class $j$ in the new binary setting—is denoted by $\hat P_j(\tilde Y_{i,j}=1)$:
...
[^ova-imbalance]: The split between observations belonging to class 1 and those not belonging to class 1 can easily become imbalanced, so one-vs-rest frequently encounters the imbalanced-data problem. See [Section 4.4](/books/deconstructing_LLM/chapter-4/4-4) for solutions.
