...
The target variable `label` remains categorical, so it must be represented by a new numerical variable. Specifically, generate a variable called `label_code` from the original data. It has two possible values: 0 represents “<=50K,” an income no greater than USD 50,000, and 1 represents “>50K,” an income greater than USD 50,000.

![Figure 4-6 | 80%](./images/4-6.webp)

Before building the model, we need a basic understanding of the data in two respects: the distribution of each variable and the relationships among variables, particularly those between the independent variables and the target.
...
More sophisticated charts can also present crosstab results. The right side of Figure 4-8 analyzes years of education (`education_num`) and the target variable. The area of each square is proportional to the value in the corresponding interval. The chart makes the conclusion clear: as years of education increase, so does the proportion of people earning more than USD 50,000.

![Figure 4-8 | 84%](./images/4-8.webp)
...
2. Lines 7 and 8 show how statsmodels specifies a model using formula syntax[^formula-syntax]. A simpler example helps explain the syntax. Suppose the formula is `label_code ~ age + age * education_num`. It specifies `label_code` as the target variable and uses two features: the original variable `age` and the derived feature `age * education_num`, which multiplies age by years of education.
...
![Figure 4-11 | 82%](./images/4-11.webp)
...
![Figure 4-12 | 70%](./images/4-12.webp)
