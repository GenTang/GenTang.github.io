...
![Figure 12-1 | 82%](./images/12-1.webp)
...
According to the principle of model composition discussed in [Section 8.3.5](/books/deconstructing_LLM/chapter-8/8-3#section-8-3-5), the core idea of neural networks is to compose smaller components into more capable models. Earlier, model components could be connected freely provided that their input and output shapes matched and their data formats were consistent. Why does that fail here? Every operation involved in those earlier compositions was differentiable[^12-1-2], allowing the combined model to use backpropagation.
...
![Figure 12-4 | 88%](./images/12-4.webp)
...
This modification allows the reward, denoted $r_t$, to be introduced as a weight in the loss, as indicated by Label 2 in Figure 12-4. Compute the loss for each text, multiply it by the text's weight—its reward—and sum the results to obtain the total loss. As discussed earlier, a larger weight places greater emphasis on reducing the corresponding loss, increasing the associated $\sum\ln P_j$ and thus the probability of generating high-reward responses. Three implementation details require attention.
...
![Figure 12-5 | 80%](./images/12-5.webp)
