...
Although gradient descent is elegant in theory, it often encounters bottlenecks in practice. To illustrate, let $L_i$ denote the model loss for observation $i$: $L_i=(y_i-ax_i-b)^2$. Summing the losses over all observations gives the overall loss $L=1/n\sum_{i=1}^{n}L_i$. In other words, the model's loss function is the average loss over its observations, a view that applies to most models[^average-loss].

Calculating the gradient of the overall loss gives $\nabla L=1/n\sum_{i=1}^{n}\nabla L_i$. Thus, the gradient of the loss function is the average of the gradients over all observations. With a large dataset, however, calculating this average takes considerable time. Stochastic gradient descent (SGD) can accelerate the process.

### 6.4.1 Algorithm Details

The central idea of stochastic gradient descent is to randomly select a small batch of observations at each iteration, calculate their gradients, and use the batch's average gradient in place of the gradient of the overall loss[^sample-gradient].
...
1. Line 2 introduces the hyperparameter `batch_size`, which controls the number of observations in each batch. An appropriate value is crucial to efficiency and stability. A value that is too large may reduce computational efficiency, while one that is too small may produce excessively noisy gradients and impair convergence. The choice must reflect the particular model, the application, and experience in the relevant field.
2. Lines 11–13 demonstrate one way to select a batch. This is also one of the principal differences between stochastic and ordinary gradient descent. Randomness can be introduced in many ways, including random-number generation. Here, only one standard approach is shown: divide the data sequentially into batches.
...
During stochastic gradient descent, the model's overall loss is normally used to monitor the algorithm. Note, however, that `loss` on line 16 of Listing 6-6 is the model loss on a mini-batch. Because it is based on only a small sample, it can fluctuate sharply across iterations and is unsuitable as the primary indicator of the algorithm's progress.
...
![Figure 6-7 | 75%](./images/6-7.webp)

### 6.4.3 Further Improvements

Recall the design of stochastic gradient descent. Although it replaces the exact full-dataset gradient with the average gradient of a mini-batch, it performs remarkably well in practice. This basic algorithm is often called vanilla SGD. The same idea can be extended by transforming the mini-batch gradients further, as shown in Figure 6-8.

![Figure 6-8 | 80%](./images/6-8.webp)

Figure 6-8 presents three approaches to processing gradients: direct use, momentum, and gradient “normalization.”

1. **Direct application**: This is the basic form of vanilla stochastic gradient descent, which directly uses the average gradient of a mini-batch to update the model parameters.
...
![Figure 6-9 | 80%](./images/6-9.webp)

3. **Gradient “normalization”**: The preceding methods use the same learning rate globally; see [Section 6.2.1](/books/deconstructing_LLM/chapter-6/6-2#section-6-2-1). This may cause different parameters to converge at different rates. To address the problem, an algorithm can perform a normalization-like transformation directly on the gradients, balancing the update rates of the parameters more effectively. Representative algorithms include Adagrad and RMSprop.

Combining momentum with gradient normalization yields a powerful optimization algorithm: Adam, or adaptive moment estimation. Adam is widely used in practice, particularly in deep learning. It combines the ideas of momentum and gradient normalization while adaptively adjusting the learning rate and momentum parameters, allowing model parameters to be updated more efficiently during training.
...
[^sample-gradient]: This is entirely reasonable mathematically. From a statistical perspective, averaging all observations is not substantially superior to random sampling. As with linear regression parameter estimates, both results are random variables whose expected value is the true gradient; the former simply has a narrower confidence interval.
