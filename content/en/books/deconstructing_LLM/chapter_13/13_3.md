...
Hidden Markov models (HMMs) resemble neural networks in several respects. Functionally, an HMM is designed specifically for sequential data and serves a role similar to that of a recurrent neural network. Structurally, it can be viewed as a simplified recurrent neural network. For model training, it uses the expectation-maximization (EM) algorithm, which has many similarities to the forward and backward passes in a neural network. Historically, hidden Markov models appeared before neural networks, and many modeling techniques were inspired by them.
...
- On a rainy day, the probabilities of going on an outing, shopping, and playing games are 10%, 40%, and 50%, respectively.
- On a sunny day, the probabilities of going on an outing, shopping, and playing games are 60%, 30%, and 10%, respectively.
...
![Figure 13-11 | 65%](./images/13-11.webp)

To abstract this example, introduce the variable $h_i$ for the weather on Day $i$, which is a hidden state, and $y_i$ for the day's activity, which is an observation. Logically, the hidden state determines the observation, but the model must infer the unobserved hidden state from the observed activity.
...
![Figure 13-12 | 74%](./images/13-12.webp)
...
This section considers only discrete hidden states. Based on the preceding discussion, the model parameters can be divided into three components: the emission distribution $P(y_i\mid h_i)$, the initial distribution $P(h_0)$, and the transition matrix $P(h_i\mid h_{i-1})$, as shown in Figure 13-13.

![Figure 13-13 | 70%](./images/13-13.webp)

Let $\boldsymbol Y=(y_0,y_1,\cdots,y_n)$ denote the observations and $\boldsymbol H=(h_0,h_1,\cdots,h_n)$ the hidden states. Their joint probability is then
...
The model parameters are estimated by maximizing the joint probability of the observations and hidden states. The observations $\boldsymbol Y$ are known, but the hidden states $\boldsymbol H$ may be unknown. If $\boldsymbol H$ is also known, estimates of the model parameters can be derived directly. If it is unknown, the expectation-maximization algorithm is required, as discussed in [Section 13.3.4](/books/deconstructing_LLM/chapter-13/13-3#section-13-3-4).
...
$S_n(h_n)$ is the maximum probability attainable for the observation sequence $\{y_1,y_2,\cdots,y_n\}$ when state $h_n$ is fixed. It can be proved mathematically that this problem can be decomposed into a series of subproblems as shown in Figure 13-14. This is the core of the Viterbi algorithm ([Complete Code](https://github.com/GenTang/regression2chatgpt/blob/en/ch13_others/viterbipy.py)).

![Figure 13-14 | 75%](./images/13-14.webp)
...
![Figure 13-15 | 76%](./images/13-15.webp)

Returning to the stock market, we first divide the model into two layers.

- The first layer contains observed stock-market features: the five-day and 20-day log returns and the five-day and 20-day log growth rates in trading value. These features are denoted by $\boldsymbol Y_i$.
...
This example uses the Shanghai Composite Index and its corresponding daily trading value from June 1, 2005, through November 28, 2023. To avoid the long-term effect of inflation on trading value, the model uses the five-day and 20-day log growth rates in trading value rather than its absolute level.

Plotting the Shanghai Composite Index under different states in separate panels produces Figure 13-16 ([Complete Code](https://github.com/GenTang/regression2chatgpt/blob/en/ch13_others/stock_analysis.ipynb)). Label 1 contains all trading data, while Labels 2, 3, and 4 contain data from the different states. From the characteristics of the data, we can infer that Label 2 represents a bear market, Label 3 a sideways market, and Label 4 a bull market.

![Figure 13-16 | 88%](./images/13-16.webp)

The model results suggest a quantitative trading strategy: buy or continue holding the asset if the current day is classified as a bull market; otherwise, sell or remain out of the market. In practice, however, live performance may fall far short of the historical backtest. When the model predicts $h_1$ from historical data, it already knows the subsequent trading data $\boldsymbol Y_2,\boldsymbol Y_3,\cdots,\boldsymbol Y_n$. The future cannot be known in reality, so Equation (13-17) must be called repeatedly to predict $h_n$ step by step rather than producing every prediction at once.
