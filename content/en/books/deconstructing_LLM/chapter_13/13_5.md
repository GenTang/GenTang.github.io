...
Although this is the first explicit discussion of singular value decomposition, previous chapters have already applied its results repeatedly. [Section 2.1.5](/books/deconstructing_LLM/chapter-2/2-1#section-2-1-5), for example, introduced a theorem stating that a low-rank matrix can be decomposed into the product of two smaller matrices. This theorem plays an important role in fine-tuning large language models and is the theoretical foundation of LoRA; see [Section 11.4.4](/books/deconstructing_LLM/chapter-11/11-4#section-11-4-4).
...
Singular value decomposition (SVD) is an important matrix factorization. Suppose $\boldsymbol A$ is an $m\times n$ matrix. It can be decomposed into the product of three matrices:
...
Although singular value decomposition is mathematically complete, artificial intelligence applications rarely require the full decomposition and usually care only about the largest few singular values. This motivates truncated singular value decomposition (truncated SVD).
...
The parameter $k$ in truncated SVD is the target rank. As with $k$ in principal component analysis, it can be chosen freely within a certain range. The appropriate value depends on the application and is usually found with a method such as grid search.

### 13.5.3 Latent Semantic Analysis

Latent semantic analysis (LSA) plays an important role in natural language processing. Based on training texts, it transforms tokens and texts into vectors that compactly encode their semantics. This conversion turns human language into vectors a model can process and lays the foundation for applications such as text classification and sentiment analysis.

1. Define the weight matrix $\boldsymbol A$ for the training data. This is an $m\times n$ matrix, where the number of rows $m$ is the number of texts and the number of columns $n$ is the vocabulary size; see [Section 10.1.1](/books/deconstructing_LLM/chapter-10/10-1#section-10-1-1). Each element is the weight of the corresponding token in a text. In practice, the weights are usually defined with TF-IDF.
...
The matrix factorization in Figure 13-25 closely resembles truncated SVD. Decomposing $\boldsymbol A$ with $k$ singular values gives $\boldsymbol A\approx\boldsymbol U_k\boldsymbol S_k\boldsymbol V_k^\mathsf{T}$. The shape of $\boldsymbol U_k$ matches that of $\boldsymbol D$, while the shape of $\boldsymbol V_k^\mathsf{T}$ matches that of $\boldsymbol T$.

- Rows of $\boldsymbol U_k$ represent texts and columns represent latent features. These features capture as much of the texts' semantics as possible, but their specific meanings are unclear—one disadvantage of latent semantic analysis.
...
Figure 13-26 shows common ways to absorb the diagonal matrix $\boldsymbol S_k$ into the two factors in practice.
...
Large-scale recommender systems are widely used on the Internet. An e-commerce platform, for example, recommends products based on a user's personal information and purchase history. From a modeling perspective, recommendation is a classification problem: for a user, classify all products as interesting or uninteresting; for a product, classify users as interested or uninterested.

Logistic regression and neural networks can solve classification problems, but they model the problem from only one perspective. From the user perspective, for example, product attributes are the input features and user preferences are encoded in the model parameters, so a separate model is required for every user. Internet applications contain enormous numbers of users and products, making it difficult for traditional classifiers to construct and train tens of thousands or even millions of models simultaneously.
