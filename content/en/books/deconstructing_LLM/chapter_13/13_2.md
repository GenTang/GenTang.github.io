...
Although decision trees are simple and transparent, they perform poorly when used alone. As described in [Section 13.1.3](/books/deconstructing_LLM/chapter-13/13-1#section-13-1-3), combining a decision tree with logistic regression can significantly improve model performance. Because this combination involves two entirely different models, however, it is difficult to provide a clean mathematical abstraction for it, preventing the approach from becoming a general-purpose solution.

The success of neural networks suggests that combining models with similar structures can unleash tremendous potential[^13-2-1]. Following this example, we can add decision trees sequentially to construct a powerful model. Each decision tree “stands on the shoulders” of the preceding model and improves it further, forming the basic idea of boosting. Decision trees also have a more classic and concise form of combination: determining the final prediction from the average output of multiple trees, known as averaging. Intuitively, boosting makes the model longer, much like deep learning in neural networks, whereas averaging makes it wider, much like increasing the width of a neural network's hidden layers.

Representative models for these two methods are random forests and gradient-boosted decision trees (GBDTs), respectively. We now examine how these models work, along with their advantages and limitations in different applications.
...
In practice, all three sources of randomness need not be implemented simultaneously. scikit-learn provides two implementations: random forests and extremely randomized trees. The former randomizes the training data and the features considered at each split, while the latter implements all three types of randomness. See the official scikit-learn documentation for the corresponding APIs.

### 13.2.2 Gradient-Boosted Decision Trees

The name gradient-boosted decision tree may sound obscure, but the term “gradient boosting” is related to the gradient descent method discussed in [Section 6.2](/books/deconstructing_LLM/chapter-6/6-2). In fact, the central idea of a gradient-boosted decision tree is an extension of gradient descent. This is not the first time we have encountered such a method. MC learning, discussed in [Section 12.3.1](/books/deconstructing_LLM/chapter-12/12-3#section-12-3-1), likewise updates its result along the gradient.
...
Why does the model's name contain “gradient”? Because the model residual is the negative gradient of the loss function. The left side of Figure 13-9 gives the derivation. In short, a gradient-boosted decision tree estimates the gradient of the loss with respect to $y$ and then updates the model using that estimate. This process resembles MC learning: both imitate gradient descent.

![Figure 13-9 | 86%](./images/13-9.webp)

With this preparation, the procedure for solving a regression problem using gradient-boosted decision trees is as follows.
...
In practice, the number of boosting stages $m$—and thus the number of decision trees—is usually chosen in advance, giving the model $F=F_0+\sum_{i=1}^{m}\gamma_i h_i$. Figure 13-10 shows the learning process ([Complete Code](https://github.com/GenTang/regression2chatgpt/blob/en/ch13_others/gbts.ipynb)). At each successive stage, a new tree attempts to correct the errors left by the preceding model, improving the overall predictive performance stage by stage.

![Figure 13-10 | 84%](./images/13-10.webp)

Although gradient-boosted decision trees were originally designed for regression, they extend naturally to classification. For a binary classification problem, the label $y_i$ may be 1 or 0. If the classification loss is defined by Equation (13-10), where $y$ is the model prediction, classification becomes completely analogous to regression.
...
[^13-2-1]: This approach is known as ensemble learning, and it is not limited to decision trees. Relatively simple machine learning models can be called weak learners; an ensemble method combines a series of weak learners into a complex model with stronger predictive performance. It resembles the broader principle of model composition, but ensemble methods refer more specifically to a class of algorithms.
