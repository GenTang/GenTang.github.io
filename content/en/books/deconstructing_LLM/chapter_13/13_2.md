...
In practice, the depth $m$ of the GBTs—the number of decision trees—is usually chosen in advance, giving the model $F=F_0+\sum_{i=1}^{m}\gamma_i h_i$. Figure 13-10 shows the learning process ([full code](https://github.com/GenTang/regression2chatgpt/blob/en/ch13_others/gbts.ipynb)). As the depth increases, each tree attempts to correct the errors left by the preceding model, improving the overall predictive performance layer by layer.
